...
const UserInsertOrUpdate = (props) => {
  ...
  React.useEffect(() => {
    ...
    ajax_alt(
      ...
      function (obj) {
        // obj holds role list from AJAX call
        console.log("role/getAll Ajax success");
        if (obj.dbError.length > 0) {
          // db error trying to read role list
          ...
          // will work.
          setUserData(
            setProp(userData, "userRoleId", obj.roleList[0].userRoleId)
          ); // FIX
          console.log(
            "set initial role id for web_user to be " +
              obj.roleList[0].userRoleId
          );

          if (action === "update") {
            //this is update, not insert, get webUser by the id
            console.log("Now getting webUser record " + id + " for the update");
            ajax_alt(
              "webUser/getById?userId=" + id,
              function (obj) {
                if (obj.errorMsg.length > 0) {
                  // obj.errorMsg holds error, e.g., db error
                  console.log(
                    "DB error trying to get the webUser record for udpate"
                  );
                  setErrorObj(setProp(errorObj, "errorMsg", obj.errorMsg));
                  //setProp = (obj, propName, propValue)
                } else {
                  // obj holds the webUser record of the given id
                  console.log(
                    "got the web user record for update (on next line)"
                  );
                  console.log(obj);
              ...
              function (ajaxErrorMsg) {
                // AJAX Error Msg from trying to read the webUser to be updated.
      ...
      function (ajaxErrorMsg) {
        // AJAX Error Msg from trying to read the role list.
        // setRoleError(msg);
        setErrorObj(setProp(errorObj, "errorMsg", ajaxErrorMsg));
      }
    );
  ...
  const validate = () => {
    ...
    console.log(
      "Here is the user data that will be sent to the insert/update API"
    );
    ...
    ajax_alt(
      ...
      function (obj) {
        // obj holds field level error messages
  ...
  return (
    <table className="user-insert-form">
      <tbody>
        <tr className="form-row">
          ...
          <td className="form-error">{errorObj.webUserId}</td>
        </tr>
        <tr className="form-row">
          ...
          <td className="form-input">
            <input
              ...
              onChange={(e) =>
                setUserData(setProp(userData, "userEmail", e.target.value))
              }
            />
          </td>
          <td className="form-error">{errorObj.userEmail}</td>
        </tr>
        <tr className="form-row">
          ...
          <td className="form-input">
            <input
              ...
              onChange={(e) =>
                setUserData(setProp(userData, "userPassword", e.target.value))
              }
            />
          </td>
          <td className="form-error">{errorObj.userPassword}</td>
        </tr>
        <tr className="form-row">
          <td className="form-label">Re-enter Password</td>
          <td className="form-input">
            <input
              type="password"
              value={userData.userPassword2}
              onChange={(e) =>
                setUserData(setProp(userData, "userPassword2", e.target.value))
              }
            />
          </td>
          <td className="form-error">{errorObj.userPassword2}</td>
        </tr>
        <tr className="form-row">
          ...
          <td className="form-input">
            <input
              ...
              onChange={(e) =>
                setUserData(setProp(userData, "userImage", e.target.value))
              }
            />
          </td>
          <td className="form-error">{errorObj.userImage}</td>
        </tr>
        <tr className="form-row">
          ...
          <td className="form-input">
            <input
              ...
              onChange={(e) =>
                setUserData(setProp(userData, "birthday", e.target.value))
              }
            />
          </td>
          <td className="form-error">{errorObj.birthday}</td>
        </tr>
        <tr className="form-row">
          <td className="form-label">Membership Fee</td>
          <td className="form-input">
            <input
              value={userData.membershipFee}
              onChange={(e) =>
                setUserData(setProp(userData, "membershipFee", e.target.value))
              }
            />
          </td>
          <td className="form-error">{errorObj.membershipFee}</td>
        </tr>
        <tr className="form-row">
          <td className="form-label">Role</td>
          <td className="form-input">
            <select
              onChange={(e) =>
                setUserData(setProp(userData, "userRoleId", e.target.value))
              }
          ...
          <td className="form-error">{errorObj.userRoleId}</td>
        </tr>
        <tr className="form-row">

...
public class DbMods {
    ...
    public static StringData getMisc(DbConn dbc, String minMemberFee, String maxBirthday) {

        StringData sd = new StringData();

        // db connection check before moving on, this just ensures we dont try to query a broken datebase connection
        sd.errorMsg = dbc.getErr(); 
        if (sd.errorMsg.length() > 0) { // cant proceed, database error
            return sd;
        }
        ...
        if (minMemberFee == null || maxBirthday == null) {
            sd.errorMsg = "one of your query values are null ";
            return sd;
        }
        
    
        // fixed 3/14/24. before was trying to pass maxBirthday into convertDecimal
        BigDecimal bdMinMemberFee = Validate.convertDecimal(minMemberFee);
        if (bdMinMemberFee == null) {
            sd.errorMsg += "Cannot get Misc user: can't convert minMemberFee to a BigDecimal.";
        }
    
        java.sql.Date dtMaxBirthday = Validate.convertDate(maxBirthday);
        if (dtMaxBirthday == null) {
            sd.errorMsg += "  Cannot get Misc User: can't convert maxBirthday to a Date.";
        }
    
        if (sd.errorMsg.length() > 0) {
            return sd; // db error or user inputs cannot be converted to correct types.
        }
        ...
        try {
            ...
            PreparedStatement stmt = dbc.getConn().prepareStatement(sql);
            stmt.setBigDecimal(1, bdMinMemberFee);
            stmt.setDate(2, dtMaxBirthday);
            ...
            } else if (sd.errorMsg.length() > 0) { // cant proceed, database error
                return sd;
            }
            else{
                sd.errorMsg = "No web user found matching that criteria or not tunneled in";
            }
            results.close();
            stmt.close();
        } catch (Exception e) {
            sd.errorMsg = "Exception thrown in getMisc: " + e.getMessage();
            return sd;
        }
    ...
    //HW 7 -- Buisness Logic of controller to logon API
    public static StringData findByEmailAndPassword(DbConn dbc, String email, String password){
        ...
        if (email == null || password == null){
            sd.errorMsg = "Email and/or Password is Not Filled in";
            return sd;
        }
        ...
        //convertign bigdceimal
        if (results.next()) {
            
            // User found, populate the StringData object
            sd.webUserId = Format.fmtInteger(results.getObject("web_user_id"));
            sd.userEmail = Format.fmtString(results.getObject("user_email"));
            sd.userPassword = Format.fmtString(results.getObject("user_password"));
            sd.userImage = Format.fmtString(results.getObject("user_image"));
        ...
        sd.errorMsg = "Database error: " + e.getMessage();
    }
